...
opt.PBAFN_warp_checkpoint = 'C:\\Users\\User\\Desktop\\yein_VTON\\checkpoints\\flow\\PBAFN_warp_epoch_031.pth'
opt.pretrain_checkpoint_D = 'C:\\Users\\User\\Desktop\\yein_VTON\\checkpoints\\flow\\PBAFN_D_epoch_031.pth'
...
for ii, data in enumerate(tqdm.tqdm(test_loader)):
    with torch.no_grad():   # 執行推論時暫時停用梯度計算
        ...
        for bb in range(bz):
            ...
            person_id = data['img_path'][bb].split('\\')[-1]
            # 儲存結果
            os.makedirs('sample/test_warping/result/test/', exist_ok = True)
            save_path = 'sample/test_warping/result/test/'+person_id+'___'+cloth_id[:-4]+'.png'
            cv2.imwrite(save_path, bgr)

            os.makedirs('sample/test_warping/seg/test/', exist_ok = True)
            save_path = 'sample/test_warping/seg/test/'+person_id+'___'+cloth_id[:-4]+'.png'
